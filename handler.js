...
module.exports = {
    ...
    try {
      ...
      try {
        ...
        if (chat) {
          ...
          if (!('sPromote' in chat)) chat.sPromote = 'تت'
          if (!('sDemote' in chat)) chat.sDemote = 'مم'
      ...
      if (opts['pconly'] && m.chat.endsWith('g.us')) return m.reply('خاص فقط')
      if (opts['gconly'] && !m.chat.endsWith('g.us')) return m.reply('المجموعات فقط')
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          m.plugin = name
          if (name === 'menu.js' && (m.sender === '212630215371@s.whatsapp.net' || m.sender === '212630215371@s.whatsapp.net')) m.reply('مرحبا مطوري!')
          ...
          //if(!user.owner || !user.rowner) return conn.reply(m.chat, `Maaf @${m.sender.split`@`[0]} \n*『 Info 』*\nKami Staff vanitasx_d ingin melakukan backup data, dan sedikit meningkatkan performa bot.\nJadi, Kami dari Staff vanitasx_d memutuskan untuk melakukan *maintenance*, jika ada yang mau ditanyakan, harap hubungi Real Owner, Terima Kasih\n\n_*Hormat Kami, Staff vanitasx_d*_`, m, { mentions: [m.sender] })
  ...
  async onCall(info) {
    ...
    if(user.call < 3){
    	...
    	return this.sendMessage(from, { text: `DONT CALL ME BITCH ${user.call}`})
    }
    await this.sendMessage(from, { text: 'DONT CALL ME AGAIN BITCH' })
